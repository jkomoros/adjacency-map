		...
		return html`
			<div class='controls' @mousemove=${this._handleControlsMouseMove}>
				...
				<select id='filenames' .value=${this._filename} @change=${this._handleFilenameChanged}>
					${this._legalFilenames.map(filename => html`<option .value=${filename} .selected=${filename == this._filename}>${displayFilename(filename)}${this._scenariosOverlays && this._scenariosOverlays[filename] ? html` (*)` : ''}</option>`)}
				</select>` : ''}
				<div>
